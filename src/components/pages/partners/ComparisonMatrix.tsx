...
const renderCell = (value: string | boolean) => {
   ...
   // Custom badges
   if (value === "Highest priority" || value === "Featured") return <span className="font-source-sans text-[13px] font-semibold text-tcl-crimson">{value}</span>;
   if (value === "Add-On") return <span className="bg-tcl-crimson/10 text-tcl-crimson rounded-full px-3 py-0.5 font-barlow-condensed font-medium text-[11px] uppercase tracking-wide">Add-On</span>;
         ...
         <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="mb-12">
               <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="font-barlow-condensed font-medium text-[13px] uppercase tracking-widest text-tcl-navy">Comparison</span>
               </div>
               ...
               <div className="flex gap-6 border-b border-tcl-gray-200 pb-1">
                  {['Overview', 'Facilities & Training', 'Partner vs Non-Partner'].map((tab, i) => (
                     <span key={i} className={`font-barlow-condensed font-medium text-[14px] uppercase tracking-wide pb-3 border-b-2 ${i === 0 ? 'text-tcl-navy border-tcl-crimson' : 'text-[#999] border-transparent'}`}>
                        {tab}
                     </span>
                  ))}
               </div>
               ...
               <table className="w-full border-collapse">
                  ...
                  <thead>
                     <tr className="bg-tcl-navy text-white">
                        ...
                        <th scope="col" className="px-4 py-5 font-barlow-condensed font-semibold text-[13px] uppercase tracking-[0.06em] border-b-[3px] border-tcl-crimson w-[15%]">
                           <div className="flex flex-col items-center gap-1">
                              <span>Founding</span>
                              <Star className="w-3 h-3 fill-tcl-crimson text-tcl-crimson" />
                           </div>
                        </th>
                  ...
                  <tbody>
                     {tableData.map((row, i) => (
                        <tr key={i} className={`border-b border-tcl-gray-200 last:border-0 hover:bg-tcl-warm-stone/20 transition-colors ${i % 2 !== 0 ? 'bg-[#FAFAF7]' : 'bg-white'}`}>
                           ...
                           <td className="px-4 py-4 text-center bg-tcl-crimson/[0.03]">{renderCell(row.founding)}</td>
            ...
            <div className="lg:hidden flex flex-col gap-8">
               {[
                  { name: "Founding Partner", color: "border-t-tcl-crimson", star: true, dataKey: "founding" },
                  ...
                  >
                     <div className="bg-tcl-gray-50 p-4 border-b border-tcl-gray-200 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                           {tier.star && <Star className="w-4 h-4 text-tcl-crimson fill-tcl-crimson" />}
                           <h3 className={`font-barlow-condensed font-bold text-[18px] uppercase ${tier.star ? 'text-tcl-crimson' : 'text-tcl-navy'}`}>{tier.name}</h3>
                        </div>
                     </div>
            ...
            <div className="mt-8 flex items-start gap-3">
               <HelpCircle className="w-5 h-5 text-tcl-crimson mt-1" />
               <div>
