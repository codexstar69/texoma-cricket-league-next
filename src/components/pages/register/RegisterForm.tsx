...
const RegisterForm = () => {
   ...
   const Input = ({ label, name, type = "text", placeholder, required, half }: any) => (
   ...
   const Select = ({ label, name, options, required, half, helper }: any) => (
      <div className={`flex flex-col ${half ? 'col-span-1' : 'col-span-2'}`}>
         <label htmlFor={name} className="font-source-sans text-[14px] font-semibold text-tcl-navy mb-2">
            {label} {required && <span className="text-tcl-crimson">*</span>}
            {!required && <span className="text-[#AAA] font-normal text-[12px] ml-1">(optional)</span>}
   ...
   const Textarea = ({ label, name, placeholder, required, helper }: any) => (
      <div className="col-span-2 flex flex-col">
         ...
         <textarea
            ...
            value={formData[name] || ''}
            onChange={handleChange}
            className="w-full rounded-xl border-[1.5px] border-tcl-light-gray bg-tcl-input-bg px-5 py-3.5 text-[16px] text-[#333] placeholder-[#BBB] font-source-sans transition-all duration-200 focus:border-tcl-gold focus:outline-none focus:ring-[3px] focus:ring-tcl-gold/10 resize-y min-h-[100px]"
         />
         {helper && <div className="mt-2 flex items-start gap-1.5 text-[12px] text-[#999] font-source-sans">{helper}</div>}
      </div>
   );

   const CheckboxGroup = ({ label, name, options, required }: any) => (
      <div className="col-span-2 flex flex-col">
               ...
               return (
                  <button
                     ...
                     className={`
                       ...
                       ${isSelected
                           ? 'bg-tcl-gold/10 border-tcl-gold text-tcl-navy'
                           : 'bg-tcl-input-bg border-tcl-light-gray text-[#666] hover:border-tcl-gold hover:bg-tcl-gold/5'}
                    `}
                     aria-pressed={isSelected}
                  >
                     {isSelected && <CheckCircle weight="fill" size={14} className="text-tcl-gold" />}
                     {opt}
                  </button>
               )
            })}
         ...
         <section className="bg-tcl-off-white py-20">
            <div className="max-w-3xl mx-auto px-6">
               ...
               >
                  ...
                  >
                     <h3 className="font-barlow-condensed font-semibold text-[14px] text-tcl-gold uppercase tracking-wide mb-4 text-center">What Happens Next</h3>
                     <div className="space-y-4">
                        <div className="flex items-start gap-3">
                           <EnvelopeSimple weight="bold" size={18} className="text-tcl-gold mt-0.5 shrink-0" />
                        ...
                        <div className="flex items-start gap-3">
                           <Clock weight="bold" size={18} className="text-tcl-gold mt-0.5 shrink-0" />
                           <p className="font-source-sans text-[15px] text-[#555]">Our team will review your registration within 2–3 business days.</p>
                        </div>
                        <div className="flex items-start gap-3">
                           <ChatCircle weight="bold" size={18} className="text-tcl-gold mt-0.5 shrink-0" />
   ...
   return (
      <section className="bg-tcl-off-white py-12 md:py-20">
         <div className="max-w-3xl mx-auto px-4 md:px-6">
            ...
            {/* Main Card */}
            <div className="bg-white rounded-2xl border border-tcl-light-gray shadow-layered p-5 md:p-8 border-t-[3px] border-t-tcl-gold relative">
               ...
               <div className="absolute top-0 right-0 left-0 h-10 flex items-center justify-center gap-4 bg-white border-b border-tcl-light-gray rounded-t-[13px] z-10 px-4">
                  <div className="flex items-center gap-2">
                     <div className={`w-2 h-2 rounded-full ${selectedType ? 'bg-tcl-green' : 'bg-tcl-gold animate-pulse'}`}></div>
                     <span className={`font-barlow-condensed font-bold text-[10px] uppercase tracking-widest ${selectedType ? 'text-tcl-navy' : 'text-tcl-gold'}`}>Type</span>
                  </div>
                  <div className="w-8 h-[1px] bg-tcl-light-gray"></div>
                  <div className="flex items-center gap-2">
                     <div className={`w-2 h-2 rounded-full border ${selectedType ? 'border-tcl-gold bg-tcl-gold' : 'border-tcl-light-gray'}`}></div>
                  ...
                  <div className="mb-10">
                     <div className="flex items-center gap-3 mb-2">
                        <div className="w-[52px] h-[52px] rounded-lg bg-tcl-navy/5 flex items-center justify-center">
                           <UserCirclePlus weight="bold" size={28} className="text-tcl-gold" />
                        </div>
                        <div>
                        ...
                        {REG_TYPES.map((type) => (
                           <button
                             ...
                             ${selectedType === type.id
                                    ? 'bg-tcl-gold/5 border-tcl-gold shadow-sm'
                                    : 'bg-tcl-input-bg border-tcl-light-gray hover:border-tcl-gold hover:bg-tcl-gold/5'}
                          `}
                              role="radio"
                              aria-checked={selectedType === type.id}
                           >
                              {selectedType === type.id && (
                                 <div className="absolute top-2 right-2 text-tcl-gold">
                                    <CheckCircle weight="fill" size={18} />
                                 </div>
                              )}
                              <type.icon weight="bold" size={24} className={`transition-colors ${selectedType === type.id ? 'text-tcl-gold' : 'text-tcl-navy group-hover:text-tcl-gold'}`} />
                              <div>
                              ...
                              <>
                                 ...
                                 <div className="bg-tcl-navy/[0.02] rounded-lg p-5 border border-tcl-light-gray/50">
                                    ...
                                    <div className="grid grid-cols-2 gap-5">
                                       ...
                                       <Textarea name="medical" label="Medical Conditions / Allergies" placeholder="Any medical conditions we should be aware of" helper={<><Lock weight="bold" size={12} className="text-tcl-gold" /> Confidential and shared only with coaching staff.</>} />
                                    </div>
                                 </div>
                              </>
                           )}
                        ...
                        >
                           ...
                           <div className="space-y-6">
                                 ...
                                 <label className="flex items-start gap-3 cursor-pointer group">
                                    <div className="relative mt-0.5">
                                       <input
                                          ...
                                          aria-required="true"
                                       />
                                       <div className="w-5 h-5 rounded-md border-[1.5px] border-tcl-light-gray bg-tcl-input-bg peer-checked:bg-tcl-gold peer-checked:border-tcl-gold transition-all duration-200 flex items-center justify-center group-hover:border-tcl-gold/50">
                              ...
                              <div className="flex items-center gap-2 text-[#999] mt-2">
                                 <Lock weight="bold" size={14} className="text-tcl-gold" />
